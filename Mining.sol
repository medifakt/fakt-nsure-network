...
contract CapitalStake is Ownable, Pausable, ReentrancyGuard {
    ...
    // Set pool lp token, Can only be called by the owner.
    function setLpToken(address _lpToken) external onlyOwner  { 

        require(_lpToken != address(0),"_lpToken is zero");
    ...
    function pendingFakt(address _user) external view returns (uint256) {
        ...
        uint256 faktReward = multiplier.mul(faktPerBlock);
    ...
    function isPending(address _user) external view returns (bool) {
        UserInfo storage user = userInfo[_user];
